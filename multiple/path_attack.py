...
"""Find branch in the tree"""
branch_list = dirname+'/branch_list.txt'
branch_dict = []
with open (branch_list,'r') as branch:
    for line in branch:
        branch_item = line.strip('\n')
        branch_dict.append(branch_item)
...
for i in range(0,attack_num):
    attacker_host = host_asn_dict[int(branch_dict[-(i+2)])] # finding the n last branch router
    prefix_item = prefix_dict[i-1]
    attacker = attacker_host+';'+prefix_item
