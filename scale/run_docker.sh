...
# echo "This is the bash file for the configuration of  four router using quagga in docker container"
 docker network create --subnet=192.9.0.0/16 p-net
...
docker run --name $router_name --net p-net --ip $host_ip --hostname=$host_name -d -v `pwd`/$as_name:/etc/quagga:rw pierky/quagga
